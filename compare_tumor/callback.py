...
# Callback to update the displayed mz value
    ...
    def update_scatter_plot(selected_compound, data_display):
        ...
        try:
            ...
            fig = go.Figure()
            x_axis = (
                melted_df["Column"].str.replace("_", " ").str.upper() if data_display == "all" else melted_df["bacteria"].str.replace("_", " ").str.upper()
            )
            y_axis = melted_df["Value"] if data_display == "all" else melted_df["value"]
            scatter_width = max(1000, len(x_axis.unique()) * 20)  # Each bacteria takes 20px, minimum width is 1000px
            ...
            fig.update_layout(
                ...
                margin=dict(l=100, r=100, t=50, b=50),  # Adjust margins
            )
            return fig
